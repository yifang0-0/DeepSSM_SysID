    ...
    def forward(self, u, y):
        ...
        for t in range(seq_len):
            ...
            if  self.mpnt_wt>=10:
                ...
                x_mean_phy = self.dyphy(u[:, :, t],x_t)
                # print(f"x_mean_phy shape: {x_mean_phy.shape}")
                # print(f"x_mean_phy dtype: {x_mean_phy.dtype}")
                # print(f"x_phi_phy weight dtype: {self.x_phi_phy[0].weight.dtype}")
                ...
                # print(f"x_phy_phi shape: {x_phy_phi.shape}")
            ...
            if self.mpnt_wt>100:
                ...
                phi_x = self.phi_x(torch.cat([x_t, x_logvar], 1))
                ...
                phi_x = self.phi_x(torch.cat([x_mean_nn, x_logvar], 1))
                y_hat_phy = self.mephy(u[:,:,t], x_t)
                ...
                y_hat = self.menn_phy(torch.cat([phi_x,y_phy_phi],1))
                loss += torch.sum((y_hat-y[:, :, t]) ** 2)
            elif self.mpnt_wt<=0:
                #pure nn
                phi_x = self.phi_x(torch.cat([x_t, x_logvar], 1))
    ...
    def generate(self, u):
        ...
        for t in range(seq_len):
            ...
            if self.mpnt_wt>100:
                ...
                y_hat_phy = self.mephy(u[:,:,t], x_t)
                y_phy_phi = self.y_phi_phy(y_hat_phy)
                y_hat[:, :, t] = self.menn_phy(torch.cat([phi_x,y_phy_phi],1))
